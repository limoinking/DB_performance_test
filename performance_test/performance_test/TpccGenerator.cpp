...
void TpccGenerator::generateWarehouses()
{
   ...
   w_csv << "w_id" << "w_name" << "w_street_1" << "w_street_2" << "w_city" << "w_state" << "w_zip"
      << "w_tax" << "w_ytd" << csv::endl;

   for (w_id = 1L; w_id<=warehouse_count; w_id++) {
      makeAlphaString(6, 9, w_name.data());
...
void TpccGenerator::generateDistricts()
{
   ...
   d_csv << "d_id" << "d_w_id" << "d_name" << "d_street_1" << "d_street_2" << "d_city" << "d_state" << "d_zip"
      << "d_tax" << "d_ytd" << "d_next_o_id" << csv::endl;
...
void TpccGenerator::generateCustomerAndHistory()
{
   ...
   c_csv << "c_id" << "c_d_id" << "c_w_id" << "c_first" << "c_middle" << "c_last" << "c_street_1" << "c_street_2" << "c_city"
      ...
      << "c_discount" << "c_balance" << "c_ytd_payment" << "c_payment_cnt" << "c_delivery_cnt" << "c_data" << csv::endl;

   h_csv << "c_id" << "c_d_id" << "c_w_id" << "d_id" << "w_id" << "since" << "h_amount" << "h_data" << csv::endl;
...
void TpccGenerator::generateItems()
{
   ...
   for (i_id = 1; i_id<=kItemCount; i_id++) {
      ...
      i_data_size = makeAlphaString(26, 26, i_data.data());
...
void TpccGenerator::generateStock()
{
   ...
   for (s_w_id = 1; s_w_id<=warehouse_count; s_w_id++) {
      ...
      for (s_i_id = 1; s_i_id<=kItemCount; s_i_id++) {
         ...
         uint32_t s_data_size = makeAlphaString(26, 26, s_data.data());
         if (orig[s_i_id]) {
...
void TpccGenerator::generateOrdersAndOrderLines()
{
   ...
   o_csv << "o_id" << "o_d_id" << "o_w_id" << "o_c_id" << "o_entry_d" << "o_carrier_id" << "o_ol_cnt" << "o_all_local" << csv::endl;
   ol_csv << "o_id" << "o_d_id" << "o_w_id" << "ol_number" << "ol_i_id" << "ol_supply_w_id" << "kNull"
   ...
   for (o_w_id = 1L; o_w_id<=warehouse_count; o_w_id++) {
      for (o_d_id = 1L; o_d_id<=kDistrictsPerWarehouse; o_d_id++) {
         ...
         for (o_id = 1; o_id<=OrdersPerDistrict; o_id++) {
            ...
            // @formatter:off
            o_csv << o_id << o_d_id << o_w_id << o_c_id << o_entry_d << to_string(o_carrier_id)
